...
/**
 * defgroup setup ModbusRtu Object Instantiation/Initialization
 * efgroup loop ModbusRtu Object Management
 * defgroup buffer ModbusRtu Buffer Management
 * defgroup discrete ModbusRtu Function Codes for Discrete Coils/Inputs
 * defgroup register ModbusRtu Function Codes for Holding/Input Registers
 *
 */
...
public:
	...
	void resetCnt(); //reset counter
    uint8_t getID(); //!<get slave ID between 1 and 247
    uint8_t getState();
    uint8_t getLastError(); //!<get last error message
    void setID(uint8_t u8id); //!<write new ID for the slave
    void setTxenPin(uint8_t u8txenpin);
    void end(); //!<finish any communication and release serial communication port
};

#endif	// #ifndef __MODBUS_RTU_H
